...
//stage1-------------------------------------
		...
		tigerPlayer = new Player(50, 50,1);
		// MovingMonster---------------------------
		...
		rockMonsters.add(new RockMonster(100, 500));
		rockMonsters.add(new RockMonster(300, 500));
		rockMonsters.add(new RockMonster(500, 500));
...
//RockThrowing-------------------------------------------
	...
	private void drawStageElements(Graphics g) {
		...
		//g.fillRect(350,450, getHeight() / 2 + 55, getHeight() / 2 + 65);
		if (isBridgeBroken) {
	...
	private void checkBearPosition() {
		...
		if (bearPlayer.getX() > 350 && bearPlayer.getX() < 450 && bearPlayer.getY() > getHeight() / 2 + 55
				&& bearPlayer.getY() < getHeight() / 2 + 65) {
	...
	private void checkCollisionsRockAttacked() {
		for (RocksOfMonsters rock : rocksOfMonsters) {
			if (rock.collidesWith(bearPlayer) || rock.collidesWith(tigerPlayer)) {
				if(rock.collidesWith(bearPlayer))bearPlayer.setHp(-1);
				else if(rock.collidesWith(tigerPlayer))tigerPlayer.setHp(-1);
				restart();
...
//-------------------------------------RockThrowingGame--------------------
	private void generateRocksOfPlayer() {
		...
		for (int i = 0; i < 5; i++) { // Generate 5 rocks
			int x = rand.nextInt(600);
			int y = 490 + rand.nextInt(110);
	...
	private void checkCollisions() {
		for (int i = monsters.size() - 1; i >= 0; i--) {
			...
			if (monster.collidesWith(bearPlayer) || monster.collidesWith(tigerPlayer)) {
				if(monster.collidesWith(bearPlayer))bearPlayer.setHp(-1);
				else if(monster.collidesWith(tigerPlayer))tigerPlayer.setHp(-1);
				restart();
